...
@Service
public class CheckAuthorsMambershipService implements JavaDelegate {
	...
	public void execute(DelegateExecution execution) throws Exception {
		log.info("CheckAuthorsMambershipService execute");
		...
		log.info("CheckAuthorsMambershipService login: {}", login);
		log.info("CheckAuthorsMambershipService magazine: {}", magazine.toString());

		execution.setVariable("autor_platio_clanarinu",
				magazine.getOpenAccessAuthors().stream().anyMatch(x -> x.getUsername().equals(login)));
	}
}
